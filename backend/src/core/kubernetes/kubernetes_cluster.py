...
class KubernetesCluster:
    def __init__(self, config: ClusterConfiguration, access_ip: str, kubeconfig_path: Path) -> None:
    ...
    def _parse_kubernetes_api_exception(self, exception: ApiException) -> tuple[str, str]:
    ...
    @retry(
        retry=retry_if_exception_type(NamespaceTerminatedError),
        wait=wait_fixed(10),
        stop=stop_after_attempt(10),
        reraise=True,
    )
    def create_namespace(self, namespace: str, skip_if_exists: bool = True) -> None:
        try:
        ...
        try:
            if helm_chart.is_oci:
                chart = await self._helm_client.get_oci_chart(
                    helm_chart.name, repo=helm_chart.repo_url, version=helm_chart.version
                )
            else:
    ...
    async def uninstall_chart(self, helm_chart: HelmChart, namespace: str | None = None) -> None:
    ...
    async def install_longhorn(self) -> None:
        self._logger.info('Installing Longhorn')
        values = {
            'defaultSettings': {'defaultDataPath': '/var/longhorn'},
            'persistence': {'defaultFsType': 'ext4', 'defaultClassReplicaCount': 2, 'defaultClass': False},
        }
        await self.install_or_upgrade_chart(longhorn_chart, values)

        self._logger.info('Installed Longhorn successfully')

    async def install_certmanager(self, domain_name: str) -> None:
    ...
    async def install_clusterautoscaler(
        self, provider_token: str, cluster_config: ClusterConfiguration, cloud_init: str
    ) -> None:
        ...
        values = {
            ...
            'extraArgs': {'scale-down-utilization-threshold': 0, 'scale-down-gpu-utilization-threshold': 0},
            'autoscalingGroups': [
                {
                    'name': f'{x.name}-autoscaled',
                    'minSize': x.autoscaling.min_nodes,
                    'maxSize': x.autoscaling.max_nodes,
                    'instanceType': x.node_type,
                    'region': x.region,
                }
                for x in cluster_config.pools
                if x.autoscaling and x.autoscaling.enabled
            ],
        }
    ...
    def expose_traefik_dashboard(
        ...
        with template_loader.render_to_temp_file(
            'traefik-basic-auth-middleware.yaml', values, 'kubernetes'
        ...
        with template_loader.render_to_temp_file(
            'traefik-dashboard-ingress-route.yaml', values, 'kubernetes'
        ) as rendered_template_file:
            try:
                self._client.install_from_yaml(rendered_template_file, with_custom_objects=True)
    ...
    def create_certificate(
        ...
        with template_loader.render_to_temp_file(
            'cert-manager-acme-certificate.yaml', values, 'kubernetes'
        ) as rendered_template_file:
            try:
                self._client.install_from_yaml(rendered_template_file, with_custom_objects=True)
    ...
    def apply_file(self, path_to_template: Path, with_custom_objects: bool = False) -> None:
    ...
    def create_secret(
        self, secret_name: str, namespace: str, data: dict[str, str], secret_type: str | None = None
    ) -> None:
        self.create_namespace(namespace)

        if secret_type == 'docker-registry':  # noqa: S105 (not a secret)
            self._client.create_docker_registry_secret(
                secret_name, data['url'], data['username'], data['password'], namespace
            )
        else:
    ...
    @classmethod
    def from_db_model(cls, cluster: Cluster) -> 'KubernetesCluster':
        return cls(
